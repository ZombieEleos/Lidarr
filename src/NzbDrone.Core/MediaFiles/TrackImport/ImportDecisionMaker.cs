...
namespace NzbDrone.Core.MediaFiles.TrackImport
{
    public interface IMakeImportDecision
    {
        ...
        List<ImportDecision<LocalTrack>> GetImportDecisions(List<string> musicFiles, Artist artist, Album album, DownloadClientItem downloadClientItem, ParsedTrackInfo folderInfo, bool filterExistingFiles, bool timidReleaseSwitching);
    }
    ...
    public class ImportDecisionMaker : IMakeImportDecision
    {
        ...
        {
            return GetImportDecisions(musicFiles, artist, null, null, folderInfo, false, false);
        }

        public List<ImportDecision<LocalTrack>> GetImportDecisions(List<string> musicFiles, Artist artist, Album album, DownloadClientItem downloadClientItem, ParsedTrackInfo folderInfo, bool filterExistingFiles, bool timidReleaseSwitching)
        {
        ...
        private ImportDecision<LocalAlbumRelease> GetDecision(LocalAlbumRelease localAlbumRelease)
        {
            ...
            if (localAlbumRelease.Distance.NormalizedDistance > 0.20)
            {
                decision = new ImportDecision<LocalAlbumRelease>(localAlbumRelease, new Rejection("Album match not close enough"));
            }
            else
            {
