...
namespace NzbDrone.Core.Parser
{
    ...
    public class FingerprintingService : IFingerprintingService
    {
        ...
        private const string _acoustIdApiKey = "1vOwZtEn";
        ...
        public bool IsSetup()
        {
            return _fpcalcPath.IsNotNullOrWhiteSpace();
        }
        ...
        public Dictionary<int, List<string>> Lookup(List<string> files, double threshold = 0)
        {
            ...
            foreach (var fileResponse in response.Fingerprints)
            {
                ...
                {
                    _logger.Debug("Found: {0}, {1}, {2}", result.Id, result.Score, string.Join(", ", result.Recordings.Select(x => x.Id)));
                }
                ...
                _logger.Debug("All recordings: {0}", string.Join("\n", ids));
